...
const ownerNumber = ["51935684086@s.whatsapp.net","994407930567@s.whatsapp.net"]
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:𝙆𝙚𝙮𝙡𝙖\n' 
            + 'ORG:𝙆;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=994407930567:+994 40 793 05 67\n' 
            + 'END:VCARD' 
...
case 'stickerwm':
	...
	reply(ind.wait())
					...
					client.sendMessage(from, pok, image, { quoted: mek, caption: `𝙔𝙪𝙣𝙤𝙗𝙤𝙩`})
					await limitAdd(sender)
					break 
...
let ui = ja[Math.floor(Math.random() * ja.length)]
                ...
                client.sendMessage(from, v, image, {quote: mek, caption: `*𝙔𝙪𝙣𝙤𝙗𝙤𝙩*`})
		        await limitAdd(sender)
                break
...
case 'bc':
       ...
       if  (!isRegistered) return reply(ind.noregis())
           ...
           case 'gogo':
           	if (isLimit(sender)) return reply(ind.limitend(pusname))
          if  (!isRegistered) return reply(ind.noregis())
                   if (args.length < 1) return reply(`[❗] Eh??\n*pone asi ${prefix}8bit uy/Keyla*`)
                  reply(ind.wait())
                   var F = body.slice(6)
				   var F1 = F.split("/")[0];
				   var F2 = F.split("/")[1]
                   anu = await getBuffer(`https://videfikri.com/api/textmaker/8bit/?text1=${F1}&text2=${F2}`)
                   client.sendMessage(from, anu, image, {caption: `*𝙔𝙪𝙣𝙤𝙗𝙤𝙩*`, quoted: mek}) 
                   break
            										            									           	          								  									           	          														            									           	          														            									           	          									
                     case 'glass':
                   if  (!isRegistered) return reply(ind.noregis()) 
                     	if (isLimit(sender)) return reply(ind.limitend(pusname))
                   if (args.length < 1) return reply(`[❗] Eh??\n*pone asi ${prefix}glass Keyla*`)
                 reply(ind.wait())
                   F = body.slice(7)				    
...
if (isLimit(sender)) return reply(ind.limitend(pusname))  
 if	(!isRegistered) return reply(ind.noregis())				
 			reply(ind.wait())
				...
				client.sendMessage(from, buffer, image, {quoted: mek, caption: '𝙔𝙪𝙣𝙤𝙗𝙤𝙩'})
				await limitAdd(sender)
				break		
       case 'nickff': 
                    				if (isLimit(sender)) return reply(ind.limitend(pusname))  
 ...
 if	(!isRegistered) return reply(ind.noregis())				
                ...
                client.sendMessage(from, h, image, {quote: mek, caption: `*𝙔𝙪𝙣𝙤𝙗𝙤𝙩*`})
		        await limitAdd(sender)
                break
                                         case 'wpanime':
                                         if (isLimit(sender)) return reply(ind.limitend(pusname))  
 if	(!isRegistered) return reply(ind.noregis())				
							...
							reply(ind.wait())
                                        ...
                                        client.sendMessage(from, l, image, {quote: mek, caption: `*𝙔𝙪𝙣𝙤𝙗𝙤𝙩*`})
					await limitAdd(sender)
                                        break
    case 'lesbi':
	if (isLimit(sender)) return reply(ind.limitend(pusname))  
...
─
│x *Nombre: GAY*
│x *Pais de origen: Per�*
│x *Nick: Keyla*
│x *wa.me/994407930567*
─ 
╰────「 *𝙔𝙪𝙣𝙤𝙗𝙤𝙩* 」────`) 
                       client.sendMessage(from, {displayname: "Confu", vcard: vcard}, MessageType.contact, { quoted: mek})
                                               break
